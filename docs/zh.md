...
因為對一個有 pane 在 copy-mode 的視窗做 resize，可能觸發 tmux 上游的換行重算迴圈、把 CPU 咬死（[tmux/tmux#4814](https://github.com/tmux/tmux/issues/4814)）。`@autosize-copy-mode-safe on`（預設）時，外掛會記下想要的尺寸，在你一離開 copy-mode 就套用——最後你還是會是正確尺寸，只是晚一點。如果你從不會踩到這個 bug，可以把它設成 `off` 讓 resize 立即發生。
...
**它會改我的 pane 佈局嗎？**
不會——它只改**視窗**尺寸。視窗一 resize，tmux 自己會等比例調整 pane；外掛不會跑 `select-layout`、也不會重新排列你的 pane。
...
這套 resize 手法——明確 `-x/-y` 收斂、per-client 尺寸、繞開 [tmux/tmux#4814](https://github.com/tmux/tmux/issues/4814) 的 copy-mode 延後／補做配對、以及給背景 `new-window -d` 用的 `TARGET_WIN` 定錨——是從作者私有 tmux resize 工具鏈抽出的通用核心。以 [MIT License](../LICENSE) 釋出。
